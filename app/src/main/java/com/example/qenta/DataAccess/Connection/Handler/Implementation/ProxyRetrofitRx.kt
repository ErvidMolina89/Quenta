package com.example.qenta.DataAccess.Connection.Handler.Implementation

import android.content.Context
import com.example.pruebavalid.DataAccess.Connection.Handler.IServiceParameters
import com.example.qenta.Base.App
import com.example.qenta.BuildConfig
import com.example.qenta.DataAccess.Connection.Handler.Interfaces.GetData
...
class ProxyRetrofitRx (private val context: Context) {
    ...
    private fun generateGetData(requestInterface: GetData): Observable<Any>{
        return when(serviceSelected?.getMethods()) {
            IServiceParameters.Methods.GET -> requestInterface.getData(App.mContext?.resources?.getString(R.string.api_key)+ serviceSelected?.getURL()!!)
